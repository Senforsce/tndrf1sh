...
func GetMaroto() core.Maroto {
	...
	m.AddRows(text.NewRow(10, "Devis DE-CMJ-00006", props.Text{
		Top:   3,
	...
	m.AddRow(7,
		text.NewCol(12, "Création de la présence de Coach Mj", props.Text{
			Top:   1.5,
			Size:  9,
			Style: fontstyle.Bold,
			Align: align.Center,
...
func getTransactions() []core.Row {
	...
	rows = append(rows, row.New(20).Add(
		text.NewCol(5, "Durée de validité : 30 jours; Taux d'escompte : Pas d'escompte applicable. Date de livraison estimée : 26-11-2024 Mode de paiement : Virement; Sous régime auto-entrepreneur, pas de TVA á facturer", props.Text{
		...
		text.NewCol(3, "EUR 11750,00", props.Text{
			Top:   20,
			Style: fontstyle.Bold,
			Size:  7,
			Align: align.Center,
	...
	rows = append(rows, row.New(5).Add(
		...
		text.NewCol(2, "Remise 2nd Client (-50%):", props.Text{
			Top:   5,
			Style: fontstyle.Bold,
			Size:  7,
			Align: align.Right,
		}),
		text.NewCol(3, "EUR -5875,00", props.Text{
			Top:   5,
			Color: getRedColor(),
			Size:  7,
			Align: align.Center,
	...
	rows = append(rows, row.New(3).Add(
		...
		text.NewCol(3, "14 Octobre 2024", props.Text{
			Top:   5,
			Style: fontstyle.Bold,
			Size:  8,
			Align: align.Center,
	...
	rows = append(rows, row.New(4).Add(
		text.NewCol(6, "20% à la commande", props.Text{
			Size:  8,
			Align: align.Left,
		}),
	))
	...
	rows = append(rows, row.New(3).Add(
		text.NewCol(2, "Echéancier possible:", props.Text{
			Style: fontstyle.Bold,
			Size:  8,
			Align: align.Left,
		}),
	...
	rows = append(rows, row.New(2).Add(
		text.NewCol(5, "20% par mois, le 1er du mois 5 fois sans frais, accompte inclus", props.Text{
			Size:  8,
			Align: align.Left,
		}),
	))
...
func getContents() [][]string {
	...
	return [][]string{
		{"CMJ-10", "Présense en ligne pour mettre en valeur Coach MJ", "14", "500 EUR", "7000 EUR"},
		{"CMJ-11", "Gestion du site Coach-Mj", "2", "500 EUR", "1000 EUR"},
		{"CMJ-14", "Système de prise de rendez-vous utilisateur", "3", "500 EUR", "1500 EUR"},
		{"CMJ-13", "Système de paiement utilisateur", "2.5", "500 EUR", "1250 EUR"},
		{"CMJ-12", "Améliorer l'expérience Utilisateur", "2", "500 EUR", "1000 EUR"},
	}
}
